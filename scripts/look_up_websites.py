...
def get_page_raw_text(driver, wait_xpath=None):
  ### Get the raw text content of the page using the body tag
  try:
    ...
    page_raw_text = driver.get_raw_text()
...
def find_webpage_type(state, county, taxAccountNumber):
  ...
  while counter:
    ...
    page_raw_text = get_page_raw_text(driver)
